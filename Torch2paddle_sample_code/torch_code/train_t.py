...
from utils_t import save_params
from dataset_t import FlatFolderDataset
from net_t import Encoder, DecoderNet
from loss_t import calc_style_emd_loss, calc_content_loss, calc_content_relt_loss, calc_style_loss
from option_t import BaseOptions
...
net_dec = DecoderNet()
net_dec.load_state_dict(torch.load("/workspace/visCVPR2021/ZBK/pre_trained/decoder_iter_10000.pth"))
net_enc = Encoder()

net_dec.to(device)
net_enc.to(device)

optimizer = torch.optim.Adam(net_dec.parameters(), lr=opt.lr)

# define dataload
train_dataset = FlatFolderDataset(opt.content_dir, opt.style_image)
test_dataset = FlatFolderDataset(opt.content_dir_test, opt.style_image)

train_iter = data.DataLoader(train_dataset, batch_size=opt.batch_size, num_workers=opt.n_threads)
test_iter = data.DataLoader(test_dataset, batch_size=1, num_workers=0)
                                         
# training 
for epoch in range(opt.epoch):
    for i, item in enumerate(train_iter):
        content_image = item[0].to(device)
        style_image = item[1].to(device)
        optimizer.zero_grad()
        loss = train(content_image, style_image, net_enc, net_dec, opt)
        print('epoch:', epoch, 'iter:', i, 'loss:%.2f'% np.array(loss.data.cpu().numpy()))
        loss.backward()
        optimizer.step()
