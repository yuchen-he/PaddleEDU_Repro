...
get_ipython().system('unzip -o -q -d  /home/aistudio/work /home/aistudio/data/data19638/insects.zip')
...
def random_expand(img,
    ...
    # 使用copy()方法，否则会修改原来gtbox的内容
    new_gtboxes = gtboxes.copy()
    new_gtboxes[:, 0] = ((gtboxes[:, 0] * w) + off_x) / float(ow)
    new_gtboxes[:, 1] = ((gtboxes[:, 1] * h) + off_y) / float(oh)
    new_gtboxes[:, 2] = gtboxes[:, 2] / ratio_x
    new_gtboxes[:, 3] = gtboxes[:, 3] / ratio_y

    return out_img.astype('uint8'), new_gtboxes


# 对原图做 随机改变亮暗、对比度和颜色等 数据增强
img_enhance, new_gtbox = random_expand(srcimg, gt_boxes)
visualize(srcimg, img_enhance)
...
img_enhance, mask, new_labels = random_crop(srcimg, gt_boxes, gt_labels)
visualize(srcimg, img_enhance)
...
visualize(srcimg, img_enhance)
srcimg.shape, img_enhance.shape
...
def random_flip(img, gtboxes, thresh=0.5):
    ...
    if random.random() > thresh:
        img = img[:, ::-1, :]
        new_gtboxes[:, 0] = 1.0 - gtboxes[:, 0]
    return img, new_gtboxes

# 对原图做 随机改变亮暗、对比度和颜色等 数据增强
img_enhance, box_enhance = random_flip(srcimg, gt_boxes)
visualize(srcimg, img_enhance)
...
def image_augment(img, gtboxes, gtlabels, size, means=None):
    ...
    return img, gtboxes.astype('float32'), gtlabels.astype('int32')

img_enhance, img_box, img_label = image_augment(srcimg, gt_boxes, gt_labels, size=320)
visualize(srcimg, img_enhance)
...
# 这里得到的img数据数值需要调整，需要除以255，并且减去均值和方差，进行数据特征标准化，再将维度从[H, W, C]调整为[C, H, W]。
...
# ### 使用飞桨高层API快速实现数据增强
# 上述代码中，我们使用numpy实现了多种数据增强方式。同时飞桨也提供了**拿来即用**的数据增强方法，详细可查阅[paddle.vision.transforms](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/transforms/functional/adjust_brightness_cn.html)模块，transforms模块中提供了数十种数据增强方式，包括亮度增强([BrightnessTransform](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/transforms/transforms/BrightnessTransform_cn.html))，对比度增强([adjust_contrast](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/transforms/functional/adjust_contrast_cn.html))，随机裁剪([RandomCrop](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/transforms/transforms/RandomCrop_cn.html))等等。更多的关于高层API的使用方法，请登录飞桨官网。
# 
...
print(reshaped_p0.shape, pred_objectness.shape, pred_objectness_probability.shape)
...
# 网络输出值是$(t_x, t_y, t_h, t_w)$，还需要将其转化为$(x_1, y_1, x_2, y_2)$这种形式的坐标表示。使用飞桨[paddle.vision.ops.yolo_box](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/ops/yolo_box_cn.html) API可以直接计算出结果，但为了给读者更清楚的展示算法的实现过程，我们使用Numpy来实现这一过程。
...
# 
# 因为有多尺度的检测，所以需要对上面的代码进行较大的修改，而且实现过程也略显繁琐，所以推荐大家直接使用飞桨 [paddle.vision.ops.yolo_loss](https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/ops/yolo_loss_cn.html) API，关键参数说明如下：
# 
...
class YOLOv3(paddle.nn.Layer):
    ...
    def get_loss(self, outputs, gtbox, gtlabel, gtscore=None,
        ...
        for i, out in enumerate(outputs): # 对三个层级分别求损失函数
            ...
            self.losses.append(paddle.mean(loss))  #mean对每张图片求平均
